...
5. **Check Memento for duplicate work:**
   - Use semantic search: `mcp__memento__semantic_search({ query: "console.log [filename] [context]", entity_types: ["HEXTRACKR:CODE:ISSUE"], limit: 5 })`
   - Check for similar issues already logged in same file
   - If duplicate found, skip to next console.log
...
- Create entity:
  ...
  - Observations:
    ...
    - `ABSTRACT: Debug console.log in [filename] at line [X]`
    - `SUMMARY: File: [path]:[line]. Log: [message]. Context: [what's being logged]. Safe to remove: [reasons]`
    - `ISSUE_ID: [HEX-XXX]`
    - `FILE_PATH: [full file path]`
    - `ISSUE_TYPE: console-log`
  - Tags:
