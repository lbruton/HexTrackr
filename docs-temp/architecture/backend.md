...
- **Schema Evolution**: The server performs idempotent `ALTER TABLE` operations on startup to add new columns to the `vulnerabilities` table, ensuring backward compatibility with older database files. It also creates the `vulnerability_snapshots`, `vulnerabilities_current`, and `vulnerability_daily_totals` tables if they don't exist.
...
The backend exposes a comprehensive REST API. For full request/response details, see the [API Reference](../api-reference/index.md).
...
- **Backup/Restore**: `GET /api/backup/stats`, `GET /api/backup/vulnerabilities`, `GET /api/backup/tickets`, `GET /api/backup/all`, `POST /api/restore`, `DELETE /api/backup/clear/:type`
