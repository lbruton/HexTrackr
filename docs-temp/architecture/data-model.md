...
Normalized vulnerability data from imports. This table is part of the legacy data model and is retained for backward compatibility.
...
| `cve` | TEXT | The CVE identifier. |
| `severity` | TEXT | The severity level. |
...
| `plugin_id` | TEXT | The ID of the scanner plugin. |
| `plugin_name` | TEXT | The name of the scanner plugin. |
| `description` | TEXT | A description of the vulnerability. |
| `solution` | TEXT | The recommended solution. |
| `vendor_reference` | TEXT | A reference from the vendor. |
...
| `vulnerability_id` | INTEGER | Foreign key to `vulnerabilities.id` or `vulnerabilities_current.id`. |
