...
class ChartThemeAdapter {
    ...
    getCSSVariables(theme) {
        try {
            ...
            const cssVars = {
                ...
                chartBackground:
                    computedStyle.getPropertyValue("--chart-background").trim() ||
                    (theme === "dark" ? "#1e293b" : "#ffffff"),
                chartGridColor:
                    computedStyle.getPropertyValue("--chart-grid-color").trim() ||
                    (theme === "dark" ? "#374151" : "#e5e7eb"),
                chartTextColor:
                    computedStyle.getPropertyValue("--chart-text-color").trim() ||
                    (theme === "dark" ? "#9ca3af" : "#6b7280"),
                chartTitleColor:
                    computedStyle.getPropertyValue("--chart-title-color").trim() ||
                    (theme === "dark" ? "#f9fafb" : "#111827"),
                chartAxisColor:
                    computedStyle.getPropertyValue("--chart-axis-color").trim() ||
    ...
    getVulnerabilityColors(theme = "light") {
        const cssVars = this.getCSSVariables(theme);
        return [cssVars.vulnCriticalColor, cssVars.vulnHighColor, cssVars.vulnMediumColor, cssVars.vulnLowColor];
    }
    ...
    async updateChartTheme(chartInstance, theme, chartId = null) {
        try {
            ...
            if (chartInstance.w && chartInstance.w.config && chartInstance.w.config.series) {
                ...
                const isVulnerabilityChart =
                    seriesNames.includes("Critical") &&
    ...
    async updateAllCharts(theme) {
        try {
            ...
            for (const [chartId, chartInstance] of this.chartInstances) {
                updatePromises.push(this.updateChartTheme(chartInstance, theme, chartId));
            }
    ...
    applyGridTheme(gridApi, theme, gridId = null) {
        try {
            ...
            console.warn(
                "AG-Grid v33 theme change requires grid recreation. Delegating to VulnerabilityGridManager.updateTheme()",
            );
