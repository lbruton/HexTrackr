...
export class AccessibilityAnnouncer {
    ...
    announce(message, options = {}) {
        try {
            ...
            const { priority = "polite", immediate = false, category = "general" } = options;
    ...
    isDuplicateAnnouncement(announcement) {
        try {
            ...
            return this.announcementQueue.some(
                (queued) => queued.message === announcement.message && queued.category === announcement.category,
            );
        } catch (error) {
    ...
    queueAnnouncement(announcement) {
        try {
            ...
            console.log(
                `Queued announcement: "${announcement.message}" (queue size: ${this.announcementQueue.length})`,
            );
