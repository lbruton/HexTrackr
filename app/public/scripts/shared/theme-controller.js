    ...
    handleSystemPreferenceChange(_event) {
        try {
            ...
            if (currentTheme === "system") {
                ...
                logger.debug(
                    "ui",
                    `System preference changed to ${newSystemPreference}, but user theme is ${currentTheme} - no change applied`,
                );
            }
        } catch (error) {
    ...
    clearOldThemeData(key, value) {
        try {
            ...
            keysToRemove.forEach((oldKey) => {
                if (oldKey !== key) {
                    // Don't remove the key we're trying to write
    ...
    generateThemeAccessibilityReport(theme = "dark") {
        try {
            ...
            const themeColors = {
                ...
                // Component colors
                "--bs-card-bg":
                    computedStyle.getPropertyValue("--bs-card-bg").trim() ||
                    computedStyle.getPropertyValue("--bs-body-bg").trim(),
    ...
    logAccessibilityReport(verbose = false) {
        try {
            ...
            logger.debug(
                ...
                "Current theme:",
                currentTheme,
                resolvedTheme !== currentTheme ? `(resolved: ${resolvedTheme})` : "",
            );
            ...
            if (resolvedTheme === "dark") {
                ...
                logger.debug(
                    "ui",
                    "Summary:",
                    `${report.summary.passed}/${report.summary.total} combinations pass WCAG AA (${report.summary.passRate}%)`,
                );
                ...
                if (report.violations.length > 0) {
                    ...
                    if (verbose) {
                        report.violations.forEach((violation, index) => {
                            logger.debug(
                                "ui",
                                ` ${index + 1}. ${violation.combination}: ${violation.ratio}:1 (needs ${violation.required}:1)`,
                            );
