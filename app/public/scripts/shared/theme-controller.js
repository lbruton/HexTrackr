...
export class ThemeController {
  ...
  /**
   * Sync theme controller state with pre-applied theme from synchronous pre-loader
   * Avoids redundant DOM manipulation and ensures consistent state without re-applying theme
   *
   * @param {string} preAppliedTheme - Theme that was already applied synchronously ('light' | 'dark')
   * @returns {void}
   */
  syncWithPreAppliedTheme(preAppliedTheme) {
    try {
      // Validate the pre-applied theme
      const validatedTheme = this.validateTheme(preAppliedTheme);
      if (!validatedTheme || !['light', 'dark'].includes(validatedTheme)) {
        console.warn(`Invalid pre-applied theme: ${preAppliedTheme}, falling back to normal initialization`);
        return;
      }

      console.log(`Syncing with pre-applied theme: ${validatedTheme}`);
      ...
      // Update any theme-dependent components (charts, grids, etc.)
      this.triggerThemeDependentUpdates(validatedTheme);

    } catch (error) {
      console.error('Error syncing with pre-applied theme:', error);
      // Fallback to normal theme application on error
      this.setTheme(DEFAULT_THEME, 'initial');
    }
  }

  /**
   * Trigger updates for theme-dependent components without applying theme to DOM
   * Used when theme is already pre-applied and we just need to notify components
   *
   * @param {string} theme - The active theme ('light' | 'dark')
   * @returns {void}
   */
  triggerThemeDependentUpdates(theme) {
    try {
      // Update AG-Grid theme if present
      if (window.modernVulnManager?.gridManager) {
        window.modernVulnManager.gridManager.forceGridRedraw();
      }

      // Update chart themes if present
      if (window.modernVulnManager?.chartManager) {
        window.modernVulnManager.chartManager.updateTheme(theme);
      }

      // Update ChartThemeAdapter if available
      if (window.ChartThemeAdapter) {
        window.ChartThemeAdapter.updateTheme(theme);
      }

    } catch (error) {
      console.warn('Error updating theme-dependent components:', error);
    }
  }
}