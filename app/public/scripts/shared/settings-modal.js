...
(function () {
    ...
    const SettingsModal = {
        ...
        async loadModalHtml() {
            try {
                ...
                const isInDocs =
                    ...
                    window.location.pathname.includes("/docs-prototype/");
                const modalPath = isInDocs
                    ? "../scripts/shared/settings-modal.html"
                    : "scripts/shared/settings-modal.html";
        ...
        setupEventListeners() {
            ...
            document
                .getElementById("importVulnerabilitiesCSV")
                ?.addEventListener("click", () => importCSV("vulnerabilities"));
    ...
    async function refreshStats() {
        try {
            ...
            if (ticketCountEl) {
                ticketCountEl.textContent = stats.tickets || 0;
            }
            if (vulnCountEl) {
                vulnCountEl.textContent = stats.vulnerabilities || 0;
            }
            if (totalCountEl) {
        ...
        try {
            ...
            if (window.preferencesSync) {
                ...
                if (credentialButton && credentialButtonText) {
                    ...
                    credentialButton.innerHTML =
                        '<i class="fas fa-check me-2"></i><span id="ciscoCredentialButtonText">Credentials Configured</span>';
                }
    ...
    async function clearCiscoCredentials() {
        if (
            !confirm("Are you sure you want to delete your Cisco PSIRT API credentials? This action cannot be undone.")
        ) {
            return;
        }
        ...
        try {
            if (window.preferencesSync && window.preferencesSync.prefsService) {
                ...
                if (result.success) {
                    ...
                    // Close modal
                    const credentialsModal = bootstrap.Modal.getInstance(
                        document.getElementById("ciscoCredentialsModal"),
                    );
                    if (credentialsModal) {
                        credentialsModal.hide();
                    }
    ...
    async function loadCiscoSyncStatus() {
        try {
            ...
            if (window.preferencesService && autoSyncCheckbox) {
                ...
                const isEnabled =
                    bgSyncResult.success && bgSyncResult.data
    ...
    async function syncCiscoNow() {
        ...
        try {
            ...
            if (response.ok && result.success) {
                ...
                showNotification(
                    `Cisco advisory sync completed: ${result.totalAdvisories} synced, ${result.matchedCount} with fixes`,
                    "success",
                );
            } else {
    ...
    async function syncKevData() {
        ...
        try {
            ...
            showNotification(
                `KEV sync completed: ${result.totalKevs} total KEVs, ${result.matchedCount} matched in your environment`,
                "success",
            );
        } catch (error) {
    ...
    function updateKevSyncStatus(status) {
        ...
        if (window.preferencesService && status.lastSync) {
            window.preferencesService
                .setPreference("kev_last_sync", status.lastSync)
    ...
    async function syncPaloNow() {
        ...
        try {
            ...
            showNotification(
                `Palo Alto sync completed: ${result.totalAdvisories} advisories, ${result.matchedCount} with fixed versions`,
                "success",
            );
        } catch (error) {
    ...
    async function togglePaloAutoSync() {
        ...
        try {
            ...
            if (window.preferencesService) {
                ...
                if (result.success) {
                    showNotification(
                        `Palo Alto auto-sync ${autoSyncCheckbox.checked ? "enabled" : "disabled"}`,
                        "info",
                    );
                } else {
                    throw new Error(result.error || "Failed to save preference");
    ...
    function updatePaloSyncStatus(status) {
        ...
        if (window.preferencesService && status.lastSync) {
            window.preferencesService
                .setPreference("palo_last_sync", status.lastSync)
    ...
    function updateUrlPreview() {
        ...
        if (!urlPreview || !instanceInput || !enabledToggle) {
            return;
        }
    ...
    async function openDatabaseBackupModal() {
        try {
            ...
            showNotification(
                `Database backup created successfully (${data.total_size_mb}MB). Files saved to disk.`,
                "success",
            );
        } catch (error) {
            ...
            fileInput.addEventListener("change", async function (event) {
                ...
                try {
                    ...
                    if (response.ok) {
                        const result = await response.json();
                        showNotification(
                            `${type} import successful: ${result.imported || data.data.length} records imported`,
                            "success",
                        );
    ...
    async function exportUserPreferences() {
        try {
            ...
            const filename = filenameMatch
                ? filenameMatch[1]
                : `hextrackr_user_preferences_${new Date().toISOString().split("T")[0]}.json`;
            ...
            const message = data.metadata
                ? `Exported ${data.metadata.exported_preferences} preferences (${data.metadata.filtered_out} sensitive items filtered)`
        ...
        try {
            ...
            input.onchange = async (event) => {
                ...
                try {
                    ...
                    const result = await response.json();
                    showNotification(
                        `Import successful: ${result.data.imported} preferences imported, ${result.data.skipped} sensitive items skipped`,
                        "success",
                    );
