...
This reference complements the [WebSocket API](../api-reference/websocket-api.md) by documenting the low-level classes and events used inside HexTrackr. The implementation lives in `server.js` (backend) and `app/public/scripts/shared/websocket-client.js` (frontend).
