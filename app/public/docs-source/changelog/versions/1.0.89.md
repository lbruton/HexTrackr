...
- 🚧 Session 3: Ticket integration + fixed version helpers (v1.0.89)
- ⏳ Session 4: Final polish + enhancements (v1.0.90)

## Testing Plan

**Manual Testing Scenarios**:

1. **Location Card Click**: Click any location card → Modal opens with correct data
2. **View Site Details Button**: Click "View Site Details" button → Modal opens
3. **Ticket Count Accuracy**: Verify batch endpoint returns correct ticket counts in grid
4. **Create Ticket Flow**: Click "Create" button → Navigate to /tickets.html with pre-filled device data
5. **View Single Ticket**: Click "View Ticket" → Navigate directly to ticket edit page
6. **View Multiple Tickets**: Click "View Tickets (N)" → Ticket picker modal shows N tickets
7. **Grid Population**: Verify 7 columns display with correct data including ticket counts
8. **Device Navigation**: Click hostname → Navigate to Device Modal (Session 2 feature)
9. **Modal Theme**: Verify dark/light theme propagates correctly to modal
10. **Severity Filters**: Click severity cards → Grid filters correctly (Session 2 feature)
...
- Location card click → Modal opens instantly
- Grid populates → Ticket counts load after ~200-500ms (batch API call)
- "Create" button → Navigates to /tickets.html with device pre-filled
- "View Tickets" → Shows picker if 2+, navigates directly if 1
