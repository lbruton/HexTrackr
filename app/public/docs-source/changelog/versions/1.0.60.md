...
#### Vulnerability Dashboard View Switching on Browser Resize (HEX-185)

**Overview**: Fixed bug where resizing the browser window forced automatic view switching between table and device cards, ignoring user's manual view selection.

**Problem**:
- Resizing browser window (maximize, widen, narrow) automatically switched views
- Desktop width (>768px) forced table view
- Mobile/tablet width (<=768px) forced device cards view
- User's manual view selection completely ignored
- Table button automatically hidden on mobile viewports
- Particularly frustrating for users who frequently resize windows

**Root Cause**:
- Window resize event listener in `vulnerability-core.js` (lines 163-186)
- Legacy code from when Table was the default view
- Attempted to restore table view when resizing from mobile back to desktop
- Became redundant when default view changed to Device Cards
- Debounced handler (250ms) forced view switches based solely on viewport width

**Solution**:
- **Removed entire window resize event listener** (deleted 24 lines)
- Users now have full manual control over view selection
- View choice persists across all window resizing operations
- Table button always visible (no automatic hiding)
- Simplified code by removing redundant responsive logic

**Result**:
- ✅ Device Cards view persists when maximizing window
- ✅ Device Cards view persists when widening/narrowing window
- ✅ Table view persists after manual switch during resize
- ✅ Table button always visible on all screen sizes
- ✅ Users control their view preference manually
- ✅ No JavaScript errors during resize operations

**Testing**:
- Verified view persistence across maximize/restore operations
- Tested with multiple window widths (desktop → tablet → mobile ranges)
- Confirmed manual view switching works correctly
- Validated no console errors during resize

**Files Modified**:
- `app/public/scripts/shared/vulnerability-core.js` - Removed lines 163-186 (resize event listener)

**Impact**:
- Better desktop-focused user experience (primary use case)
- Cleaner, simpler code (24 lines removed)
- Users maintain control over their view preference
- No forced view switching on any screen size

**Issue**:
- [HEX-185](https://linear.app/hextrackr/issue/HEX-185) - BUG: Vulnerability Dashboard Resets to Table View on Browser Resize

---

#### Device Modal IP Address Display (HEX-174)

**Overview**: Fixed device security modal to display IP addresses from vulnerability records instead of showing "N/A".

**Problem**:
- Device modal IP Address field always displayed "N/A"
- IP addresses were available in vulnerability data but not shown
- Device aggregation only captured first vulnerability's IP, which could be empty

**Root Cause**:
- Device aggregation in `vulnerability-data.js` set `ipAddress: vuln.ip_address || ""` from only the **first** vulnerability during grouping
- If first vulnerability had empty IP but later vulnerabilities had IPs, they were ignored
- Modal displayed pre-aggregated `device.ipAddress` which could be empty string

**Solution**:
- Added logic to search through all vulnerability records for device at display time
- Uses `.find(v => v.ip_address)` to locate first vulnerability with non-empty IP
- Mirrors AG-Grid table behavior which directly displays `vuln.ip_address` from each record
- Falls back to `device.ipAddress` → `"N/A"` if no vulnerabilities have IP addresses

**Implementation** (`/app/public/scripts/shared/device-security-modal.js:76-78`):
```javascript
...
**Result**:
- Device modal now displays IP addresses when available in vulnerability data
- More robust than original approach (checks ALL vulnerabilities, not just first)
- Consistent with how AG-Grid table shows IP addresses
- Gracefully handles empty/missing data with "N/A" fallback

**Future Enhancement**:
- This is a short-term fix using client-side data already in memory
- Backend refactor to database-first device queries will provide more efficient IP retrieval
- Related to ongoing performance optimization research

**Files Modified**:
- `app/public/scripts/shared/device-security-modal.js` - Added IP extraction from vulnerability records

**Issue**:
- [HEX-174](https://linear.app/hextrackr/issue/HEX-174) - Device modal IP address field shows "N/A"
