...
**For experienced RHEL admins who want the essentials:**

```bash
# 1. Configure static IP (if needed)
nmcli connection modify <interface> ipv4.addresses 192.168.1.80/24 ipv4.gateway 192.168.1.1 ipv4.method manual
...
# 4. Copy files to /opt/hextrackr/ and create .env
cd /opt/hextrackr
cat > .env <<EOF
NODE_ENV=production
PORT=8080
...
USE_HTTPS=false  # ⚠️ CRITICAL: false when using nginx reverse proxy
TRUST_PROXY=true
...
**🚨 CRITICAL GOTCHAS** (must-read before deploying):
- **Docker volumes**: Let Compose create them (DO NOT use `docker volume create`)
- **USE_HTTPS**: Must be `false` with nginx reverse proxy (502 errors if true)
- **Rate limiting**: Use `NODE_ENV=development` for testing/demos
- **Admin login**: Default is `admin` / `admin123!` (change immediately)
...
### Step 4.1: Understanding Reverse Proxy HTTPS Architecture

**⚠️ CRITICAL - HTTPS Configuration:**

When using nginx as a reverse proxy (production deployment), the HTTPS termination occurs at nginx, NOT at the Node.js application.

**Architecture Diagram:**
```
┌─────────┐     HTTPS      ┌───────┐      HTTP       ┌──────────┐
│ Browser │ ────────────> │ nginx │ ────────────> │ Node.js  │
│         │   (port 443)   │       │   (port 8080)  │ App      │
└─────────┘                └───────┘                 └──────────┘
                            SSL Termination         USE_HTTPS=false
```

**Configuration Rules:**
- **Production with nginx**: `USE_HTTPS=false` in Node.js app
- **Development without nginx**: `USE_HTTPS=true` in Node.js app
- **NEVER**: Set `USE_HTTPS=true` when using nginx reverse proxy

**Common Error:**
If you set `USE_HTTPS=true` with nginx reverse proxy, you will get **502 Bad Gateway** errors because:
- nginx tries to connect via HTTP (`proxy_pass http://hextrackr_backend`)
- Node.js app is listening on HTTPS
- Connection refused → 502 error

### Step 4.2: Create Environment File with Session Secret
...
# HTTPS/SSL SETTINGS
# ==============================================================================
# CRITICAL: Set to false when using nginx reverse proxy (production)
# Set to true only for development without nginx
USE_HTTPS=false
...
**⚠️ CRITICAL - Volume Naming Convention:**

Docker Compose automatically prefixes volume names with the project name. **DO NOT** manually create volumes with `docker volume create` as this will create volumes without the prefix, leading to database mismatch issues.

**WRONG (will cause issues):**
```bash
docker volume create hextrackr-database  # Creates volume without prefix
```

**CORRECT:**
```bash
# Let Docker Compose create volumes automatically
# The volume will be named: hextrackr_hextrackr-database (with prefix)
# No manual volume creation needed!
```

**If you have ALREADY created a manual volume**, delete it now:
```bash
# Check for manually created volume
docker volume ls | grep hextrackr-database

# If you see BOTH volumes listed, delete the one WITHOUT prefix:
# hextrackr-database          ← DELETE THIS (no prefix)
# hextrackr_hextrackr-database ← KEEP THIS (has prefix)

docker volume rm hextrackr-database  # Delete manual volume
```

**Verify correct volume naming:**
```bash
# Start compose to create volumes automatically
docker compose up --no-start

# Verify volume has correct name with prefix
docker volume ls | grep hextrackr
# Should show: hextrackr_hextrackr-database
```
...
**⚠️ CRITICAL - Use Correct Volume Name:**

If you have a database backup to restore, you MUST use the correct Docker Compose volume name with prefix.
...
# CRITICAL: Get the CORRECT volume name (with project prefix)
VOLUME_NAME=$(docker volume ls --filter "name=hextrackr.*database" --format "{{.Name}}")
echo "Using volume: $VOLUME_NAME"
# Should output: hextrackr_hextrackr-database

# Copy backup to CORRECT named volume
docker run --rm \
  -v $VOLUME_NAME:/data \
  -v /opt/hextrackr/backups:/backups \
  alpine sh -c "cp /backups/hextrackr-backup-YYYYMMDD-HHMMSS.db /data/hextrackr.db"

# Verify file was copied to CORRECT volume
docker run --rm -v $VOLUME_NAME:/data alpine ls -lh /data/
# Should show ~837MB file (or your backup size)

# Verify tables exist (critical check!)
docker run --rm -v $VOLUME_NAME:/data alpine sh -c \
  "apk add sqlite > /dev/null 2>&1 && sqlite3 /data/hextrackr.db '.tables'" | grep -i audit_log_config
# Should show: audit_log_config and audit_logs
```
...
- [ ] Volume name is `hextrackr_hextrackr-database` (with prefix)
- [ ] Database file is ~837MB (or expected size)
- [ ] Tables exist: `audit_log_config`, `audit_logs`, `vulnerabilities_current`, etc.
- [ ] No extra volumes without prefix exist
...
echo ""
echo "=== Database Size ==="
docker run --rm -v hextrackr_hextrackr-database:/data alpine du -sh /data/hextrackr.db
echo ""
echo "=== Critical Tables Check ==="
docker run --rm -v hextrackr_hextrackr-database:/data alpine sh -c \
  "apk add sqlite > /dev/null 2>&1 && sqlite3 /data/hextrackr.db \
  \"SELECT name FROM sqlite_master WHERE type='table' AND name IN ('audit_log_config','audit_logs','vulnerabilities_current')\""
echo ""
echo "All three tables should be listed above!"
```
...
### CRITICAL: Rate Limiting Issues (HTTP 429)

**Problem:** Login fails with "network error" or "connection timeout"

**Symptoms**:
- Browser console shows: `Login error: SyntaxError: JSON.parse`
- Network tab shows: `POST /api/auth/login [HTTP/2 429]`
- Application appears to "hang" during login

**Root Cause**:
HexTrackr makes many API requests during initial page load (before browser caching is established). Production mode enforces strict rate limits that can be exhausted quickly:
- **OLD Production limit**: 60 requests per 15 minutes ❌ (too low)
- **NEW Production limit**: 1,000 requests per 15 minutes ✅ (appropriate)
- **Development limit**: 10,000 requests per minute ✅ (for testing)

**Diagnosis**:
```bash
# Check current environment mode
docker exec hextrackr-app env | grep NODE_ENV

# Check nginx logs for 429 errors
docker compose logs nginx --tail 50 | grep 429

# Example 429 error in logs:
# 192.168.1.231 - - [20/Oct/2025:07:14:09 +0000] "POST /api/auth/login HTTP/2.0" 429 55
```

**Solution 1: Use Development Mode** (for testing/presentations):
```bash
# Edit docker-compose.yml
vi /opt/hextrackr/docker-compose.yml
...
**Solution 2: Increase Production Limits** (for actual production):
```bash
# Edit constants file
vi /opt/hextrackr/app/utils/constants.js

# Find line ~59 and change:
const RATE_LIMIT_MAX_REQUESTS = IS_PRODUCTION ? 60 : 10000;
# To:
const RATE_LIMIT_MAX_REQUESTS = IS_PRODUCTION ? 1000 : 10000;

# Restart to apply changes
docker compose restart hextrackr
```

**Solution 3: Clear Rate Limit** (immediate relief):
```bash
# Simply restart the container to reset rate limit counters
docker compose restart hextrackr
...
**Prevention**:
- **For testing/demos**: Use `NODE_ENV=development`
- **For production**: Use increased limits (1,000/15min minimum)
- **Monitor**: Check nginx logs for 429 errors after deployment
- **Document**: Inform users that initial page loads make many requests

**Additional Notes**:
- Rate limiting is **IP-based** (per client IP address)
- **Cache HITs are excluded** from rate limit counting
- After initial page load and caching, request volume drops significantly
- The 429 response is plain text, causing JSON parse errors in frontend
...
|---------|-------|----------|
| **502 Bad Gateway** | `USE_HTTPS=true` with nginx | Set `USE_HTTPS=false` in .env (Step 4.2) |
| **Login fails with 429** | Rate limiting too strict | Use `NODE_ENV=development` or increase limits (Troubleshooting) |
| **App restarts constantly** | Wrong database volume | Delete manual volume, let Compose create it (Step 6.2) |
| **"No such table" error** | Database not copied to volume | Verify volume name has prefix: `hextrackr_hextrackr-database` (Step 6.4) |
| **Can't SSH after static IP** | Gateway misconfigured | Check gateway with `ip route show` (Step 1.0) |
| **Firewall blocks app** | SSH rule missing | Add SSH explicitly: `firewall-cmd --add-service=ssh` (Step 1.3) |
| **SELinux blocks Docker** | Wrong contexts | Apply contexts: `sudo restorecon -Rv /opt/hextrackr` (Step 3.3) |
...
| 1.1     | 2025-01-20 | Added Quick Start, fixes for volume naming, USE_HTTPS, rate limiting |
