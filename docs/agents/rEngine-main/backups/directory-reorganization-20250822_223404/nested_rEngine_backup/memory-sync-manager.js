...
class MemorySyncManager {
  ...
  async syncToMCP(memoryData) {
    ...
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('MCP sync timeout')), 5000)
    );
    ...
    const syncPromise = (async () => {
      try {
        ...
        if (memoryData.system_state.sync_status) {
          memoryData.system_state.sync_status.sync_failures = 
            (memoryData.system_state.sync_status.sync_failures || 0) + 1;
        }
        return false;
      }
    })();
    ...
    try {
      ...
      console.warn('⚠️  MCP sync timed out or failed (non-critical):', error.message);
      return false;
    }
  }
  ...
  async preCommitSync() {
    ...
    // Add overall timeout for the entire pre-commit process
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('Pre-commit sync timeout')), 10000)
    );
    ...
    const syncProcess = (async () => {
      try {
        ...
        // Attempt MCP sync with timeout
        const syncSuccess = await this.syncToMCP(memory);
        
        if (syncSuccess) {
          console.log('✅ Pre-commit sync completed successfully');
        } else {
          console.log('⚠️  Pre-commit sync completed with warnings');
        }
    ...
    try {
      return await Promise.race([syncProcess, timeoutPromise]);
    } catch (error) {
      console.warn('⚠️  Pre-commit sync timed out, continuing with commit:', error.message);
