...
# ============================================================================
# PHASE 3: STANDALONE MCP SERVER INITIALIZATION
# ============================================================================
echo -e "${CYAN}PHASE 3: Standalone MCP Server Initialization...${NC}"

# Start standalone MCP manager (memory, context7 - no VS Code dependency)
echo -e "${YELLOW}🔌 Starting Standalone MCP Manager...${NC}"
if [ -f "./standalone-mcp-manager.cjs" ]; then
    node standalone-mcp-manager.cjs &
    MCP_MANAGER_PID=$!
    
    # Wait for MCP servers to be responsive
    echo -e "${YELLOW}⏳ Waiting for standalone MCP servers to start...${NC}"
    sleep 5
    
    # Check if MCP manager is running
    if ps -p $MCP_MANAGER_PID > /dev/null; then
        echo -e "${GREEN}✅ Standalone MCP Manager running (PID: $MCP_MANAGER_PID)${NC}"
        echo -e "${GREEN}   • Memory MCP Server (Port 8080)${NC}"
        echo -e "${GREEN}   • Context7 MCP Server (Port 8081)${NC}"
        echo -e "${GREEN}   • Zero VS Code extension dependencies${NC}"
    else
        echo -e "${RED}❌ Standalone MCP Manager failed to start${NC}"
        exit 1
    fi
else
    echo -e "${YELLOW}⚠️  Standalone MCP manager not found - falling back to integrated method${NC}"
    # Fallback to integrated method
    if [ -f "./integrated-mcp-manager.cjs" ]; then
        node integrated-mcp-manager.cjs &
        MCP_PID=$!
        
        # Wait for MCP server to be responsive
        echo -e "${YELLOW}⏳ Waiting for integrated MCP server to become responsive...${NC}"
        sleep 5
        
        if ps -p $MCP_PID > /dev/null; then
            echo -e "${GREEN}✅ Integrated MCP Manager fallback running (PID: $MCP_PID)${NC}"
        else
            echo -e "${RED}❌ MCP server fallback failed to start${NC}"
            exit 1
        fi
    else
        echo -e "${YELLOW}⚠️  No MCP server managers found${NC}"
    fi
fi
...
sync_memory_with_retry() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        if bash /Volumes/DATA/GitHub/rEngine/memory-sync-automation.sh manual; then
...
launch_scribe_console() {
    echo -e "${YELLOW}   🤖 Launching Enhanced Scribe Console in Terminal.app...${NC}"
    
    # Use the existing working AppleScript launcher
    if [ -f "./launch-smart-docs.applescript" ]; then
        chmod +x "./launch-smart-docs.applescript"
        osascript "./launch-smart-docs.applescript"
        return $?
    else
        echo -e "${RED}   ❌ AppleScript launcher not found${NC}"
        return 1
    fi
}
