...
if [ "$STARTUP_MODE" = "legacy" ]; then
    ...
    # Check Ollama availability (don't kill desktop version)
    ...
    # Check for existing Enhanced Scribe Console
    echo -e "${YELLOW}🔍 Checking Enhanced Scribe Console processes...${NC}"
    if pgrep -f "enhanced-scribe-console" > /dev/null; then
        echo -e "${YELLOW}⚠️  Enhanced Scribe Console found - killing existing processes...${NC}"
        pkill -f "enhanced-scribe-console"
        sleep 2
    fi
...
EOF
    ...
    if [ "$AGENT_TYPE" != "" ]; then
        ...
        if [ -f "rEngine/universal-agent-init.js" ]; then
            node rEngine/universal-agent-init.js --agent-type "$AGENT_TYPE" --auto --non-interactive
        else
