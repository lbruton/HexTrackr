...
echo -e "${YELLOW}🔌 Starting MCP Memory Server...${NC}"
    if [ -f "/Volumes/DATA/GitHub/rEngine/start-mcp-servers.sh" ]; then
        bash /Volumes/DATA/GitHub/rEngine/start-mcp-servers.sh &
    MCP_PID=$!
...
# Memory sync with enhanced retry logic
sync_memory_with_retry() {
    local max_attempts=8  # Increased from 3 to 8 attempts
    local attempt=1
    local wait_time=5
    
    echo -e "${YELLOW}🧠 Enhanced memory sync with system readiness detection...${NC}"
    
    while [ $attempt -le $max_attempts ]; do
        echo -e "${YELLOW}🧠 Memory sync attempt $attempt/$max_attempts...${NC}"
        
        # Check if MCP server is ready before attempting sync
        if docker ps --filter "name=mcp-server" --filter "status=running" | grep -q "healthy\|Up"; then
            echo -e "${GREEN}   MCP server detected as ready${NC}"
        else
            echo -e "${YELLOW}   MCP server still initializing, waiting...${NC}"
            sleep $wait_time
            ((attempt++))
            continue
        fi
        
        # Attempt the memory sync
        if bash /Volumes/DATA/GitHub/rEngine/memory-sync-automation.sh manual; then
            echo -e "${GREEN}✅ Memory synchronization successful on attempt $attempt${NC}"
            return 0
        else
            echo -e "${YELLOW}⚠️  Memory sync attempt $attempt failed${NC}"
            if [ $attempt -lt $max_attempts ]; then
                echo -e "${YELLOW}   Waiting $wait_time seconds before retry (system may still be initializing)...${NC}"
                sleep $wait_time
                # Gradually increase wait time for later attempts
    ...
    echo -e "${YELLOW}   Note: Enhanced Scribe Console may still provide context on next launch${NC}"
    return 1
}
...
tell application "Terminal"
    ...
    try
    set newWindow to do script "cd /Volumes/DATA/GitHub/rEngine && echo '🚀 Starting Enhanced Scribe Console...' && node enhanced-scribe-console.js"
