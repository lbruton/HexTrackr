...
// Generates high-quality documentation for any given script using Google's powerful Gemini API.
...
class DocumentManager {
    constructor() {
        ...
        this.provider = this.docManagerConfig.api.provider;
    ...
    async generate(filePath) {
        ...
        console.log(`🧠 Using ${this.provider === 'groq' ? 'Groq' : 'Gemini'} model: ${this.model}`);
        ...
        try {
            ...
            if (this.provider === 'groq') {
                ...
                const generatedDoc = response.data.choices[0].message.content;
                await this.saveDocumentation(absoluteFilePath, generatedDoc);
            } else {
                // Gemini API call
                response = await axios.post(`${this.apiEndpoint}?key=${this.apiKey}`, {
                    contents: [{
                        parts: [{
                            text: prompt
                        }]
