Please Generate a weekly insights report covering the last 7 days of development.
...
4. Use tag-based searches to find related work (spec tags, breakthrough tags, etc.)
5. Provide the User with the information using the following template

**Enhanced Search Strategy (Using Tags):**
- Search for `"week-${currentWeek}-${year}"` tagged sessions
- Filter by `"project:[PROJECT_NAME]"` for specific projects
- Look for `"breakthrough"`, `"lesson-learned"`, `"pattern"` tags
- Check `"spec:XXX"` tags to group work by specifications
- Use `"completed"` vs `"in-progress"` to show progress
