...
// Example: "HEXTRACKR-HANDOFF-20250907-143045"
```
...
// Add handoff-specific tags per TAXONOMY.md
mcp__memento__add_tags({
  ...
  tags: [
    ...
    "handoff",                       // Always include for handoffs
    "spec:[ACTIVE_SPEC]",           // Current spec being worked on
    "[CATEGORY]",                    // Work area: frontend, backend, database, etc.
    "in-progress",                   // Handoffs are typically for in-progress work
    `week-${getWeekNumber()}-${new Date().getFullYear()}`,  // Temporal tag
    "v[CURRENT_VERSION]",            // Current version
    "[BLOCKED_TAG]"                  // Optional: "blocked" if waiting on something
  ]
})
...
```json
{
  "handoff_id": "HEXTRACKR-HANDOFF-20250907-143045",
